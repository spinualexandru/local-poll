...
import type { CastVoteBody, VoteCountResponse, VoteResponse, VotesResponse } from "../types/vote.ts";
...
export class VoteController extends Controller {
    constructor() {
        ...
        this.registerRoute('/votesCount', 'get', this.getOptionsVotesCount.bind(this));
    ...
    public async castVote(
        ...
        try {
            ...
            if (isNaN(pollId) || isNaN(optionId) || (userId !== undefined && isNaN(userId))) {
                return { 
                    success: false, 
    ...
    public async getOptionsVotesCount(
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch vote counts';
            return { success: false, error: errorMessage };
        }
    }
